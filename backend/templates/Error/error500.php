    ...
    <div class="max-w-md w-full">
        <div class="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl border border-blue-200/50 p-8 text-center">
            ...
            <div class="flex justify-center mb-6">
                <div class="w-20 h-20 bg-gradient-to-br <?= $colorFrom ?> <?= $colorTo ?> rounded-2xl shadow-lg flex items-center justify-center">
            ...
            <!-- Error Code -->
            <div class="mb-4">
                <h1 class="text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r <?= $colorFrom ?> <?= $colorTo ?>">
                    <?= h($code) ?>
                </h1>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-6 text-left max-h-60 overflow-auto">
                <p class="text-xs text-gray-700 font-semibold mb-2">Error Details:</p>
                <p class="text-xs text-gray-500 font-mono break-all">
                    <strong>URL:</strong> <?= h($url) ?>
                </p>
                <p class="text-xs text-gray-600 font-mono break-words mt-2">
                    <strong>Message:</strong> <?= h($error->getMessage()) ?>
                </p>
                <details class="mt-2">
                    <summary class="text-xs text-gray-600 font-mono cursor-pointer">Stack Trace</summary>
                    <p class="text-xs text-gray-500 font-mono mt-2 whitespace-pre-wrap break-words">
                        <?= h($error->getTraceAsString()) ?>
                    </p>
                </details>
            </div>
            <?php endif; ?>
